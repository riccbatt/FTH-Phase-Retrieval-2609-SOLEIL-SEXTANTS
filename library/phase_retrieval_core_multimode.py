...
def phase_retrieval_algorithm(
    ...
    schedule through alpha, optional Richardson-Lucy parameters, and optionally
    a multimode reconstruction through ``recipe["Nmodes"]``.

    ``pos`` and ``neg`` are always 2D measured hologram intensities. The support,
    starting images, reconstructed images, and gamma may be either 2D or modal
    3D arrays with shape ``(Nmodes, nx, ny)``.
    ...
    Returns
    -------
    retrieved_fc_p, retrieved_fc_n : np.ndarray or None
        Latest reconstruction obtained with ``use_RL=False``.
    retrieved_pc_p, retrieved_pc_n : np.ndarray or None
        Latest reconstruction obtained with ``use_RL=True``.
    ...
    if pos_input.ndim != 2 or neg_input.ndim != 2:
        raise ValueError("pos and neg must be 2D measured hologram intensities.")
    ...
    if supportmask.ndim == 2:
        if supportmask.shape != pos_input.shape:
            raise ValueError("2D supportmask must have the same shape as pos/neg.")
    elif supportmask.ndim == 3:
        if supportmask.shape[1:] != pos_input.shape:
